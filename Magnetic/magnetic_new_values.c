...
void magnetic_start_detection(void) {
    ...
    // clear the moving average filter buffer and reset it to the baseline
    total = 0;
    for (int i = 0; i < FILTER_SIZE; i++) {
        readings[i] = baseline;
        total += baseline;
...
int magnetic_poll(void) {
    ...
    if (delta < THRESHOLD) { // if sensor reading is below baseline by threshold amount or more, consider instance as Up
        up_count++;
        down_count = 0;
        idle_count = 0;
    }
    else if (delta > -THRESHOLD) { // if sensor reading is above baseline by threshold amount or more, consider instance as Down
        down_count++;
        up_count = 0;
        idle_count = 0;
    }
